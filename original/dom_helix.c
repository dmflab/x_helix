/* This version determines helix packing in one domain of one chain */
...
#define DLINLEN 150
#define MAXHELICES 100                /* max helices in a domain - set between 50-100 */
...
struct DOM
{
   char protein_id[6];            /* five letters, pdb code + chain letter */
   int dom_number;                /* first = 1, second = 2 etc. */
   int segments;                  /* number of subdomains */
   float residue[MAX_SEGMENTS*2];   /* residue numbers for segment/domain borders */
};
...
{
   int helix_no;
   char chain[7];                      /* chain = PDB_ID + chain letter + domain number */
...
struct DOM* read_doms(int *domains_total, char *code, char *cathfile);
struct HELIX* read_helices(FILE *fptr, struct DOM*, int *helices_total, int *domains_total, char *code);
struct ATOM** read_atom(FILE *fp, struct HELIX*, int *helices_total, int *helices_atom_total, char *code);
...
int main(void)     
{
   ...
   char output_file1[25]="helix_packing_pair.txt";
   char output_file2[20]="helix_shape.txt";
   char code[7];                         /* code is going to be pdb_id + chain + domain number */
   char pdb_id[5];                       
   char dsspfile[50];
   char PDBDIR[39];
   char DSSPDIR[41];
   ...
   fprintf(fpo1,"Domain\tHelix1\tHelix2\tCont 1\tCont 2\tGlobal Angle\tLocal Angle\tDistance\tCovalnt\tElectro\tH-Bond\tVDW\n");
   ...
   fprintf(fpo2,"Domain\tHelix\tLength\tGeom\tMax Bending Angle\n");

   fclose(fpo2);

   printf("Input filename read by taking first six characters of each line.\n");
   printf("Six characters: <pdbcode + chainletter + domain number>\n\n");
   ...
   printf("Please enter the CATH domain definition filename (or leave blank for domlist.v2.4): ");
   fgets (domfile,20,stdin);
   if(domfile[strlen(domfile)-1]=='\n') domfile[strlen(domfile)-1]='\0';
   if(!strlen(domfile)) strcpy(domfile,"domlist.v2.4");
   ...
   while(!feof(fp1)) 
   {
      ...
      printf("Analysis Of %s in progress",code);
      ...
      domain=read_doms(&domains_total, code, domfile);

      helix=read_helices(fptr, domain, &helices_total, &domains_total, code);
      ...
      helix_atom=read_atom(fp, helix, &helices_total, &helices_atom_total, code);
      ...
      for(i=0;i<helices_total;i++)
      {
         fprintf(fpo,"domain: %s, helix: %d, start residue: %.2f, last residue: %.2f\n",helix[i].chain,helix[i].helix_no,helix[i].residue_numbers[0],helix[i].residue_numbers[helix[i].residues_total-1]);    
      ...
      for(j=0;j<helices_total;j++)
      {
         for(i=0;i<=j;i++)             
         {
            if((helix_pair[i][j].packed==1) && (helix[i].residues_total>=4) && (helix[j].residues_total>=4))
            {
               fprintf(fpo1,"%s\t%d\t%d\t%d\t%d\t",helix[i].chain,helix[i].helix_no,helix[j].helix_no,helix_pair[i][j].h1_residues,helix_pair[i][j].h2_residues); 
      ...
      for(i=0;i<helices_total;i++)
      {
         fprintf(fpo2,"%s\t%d\t%d\t",helix[i].chain,helix[i].helix_no,helix[i].residues_total);
...
struct DOM* read_doms(int *domains_total, char *code, char *cathfile)
{
   ...
   FILE *fp;
   char pdb_id[6];                    /* this is pdb code plus chain letter */             
   char line[DOMLEN];
   char *token;
   char sepstring[]=" -";
   int doms_total;
   ...
   if(code[5]!='0')       /* if there is a domain number, then continue... */
   {
      if((fp=fopen(cathfile,"r"))==NULL)
      {
         printf("Error opening %s\n",cathfile);
         exit(1);
      }
      ...
      while(!feof(fp)) 
      {
         fgets(line, DOMLEN, fp);

         if(line[strlen(line)-1]=='\n') line[strlen(line)-1]='\0';
      
         if ( (line[0]=='#') || (line[0]==' ') || (line[0]=='\n') ) continue;

         strncpy(pdb_id,line,5);
         pdb_id[5]='\0';

         if(!strncmp(pdb_id,code,5))
         {
            ...
            if ( strlen(token) < 6 )
            {
               printf("Error 1: reading domain boundary file %s\n",cathfile);
               fclose(fp);
               exit(1);
            }
            ...
            if ( !token || (strlen(token) < 3) || (sscanf(token+1,"%d",&doms_total) != 1) )
            {
               printf("Error 2: reading domain boundary file %s\n",cathfile);
               fclose(fp);
               exit(1);
            }

            /* memory allocation for an array of DOM structures */

            domain=(struct DOM *) calloc(doms_total,sizeof(struct DOM));

            *domains_total=doms_total;
            ...
            if ( !token )
            {
               printf("Error 3: reading domain boundary file %s\n",cathfile);
               fclose(fp);
               exit(1);
            }

            for(i=0; i<doms_total; i++)
            {
               strcpy(domain[i].protein_id,pdb_id);
       
               domain[i].dom_number=i+1;

               token = strtok(NULL,sepstring);

               if ( !token || (sscanf(token,"%d",&segs_total) != 1) )
               {
                  printf("Error 4: reading domain boundary file %s\n",cathfile);
                  fclose(fp);
                  exit(1);
               }

               if ( segs_total > MAX_SEGMENTS )
               {
                  printf("Error 5: too many segments in domain\n");
                  fclose(fp);
                  exit(1);
               }
    
               domain[i].segments=segs_total;

               for (j=0; j<segs_total*2; j+=2)
               {
                  token = strtok(NULL,sepstring);
          
                  if ( !token || (sscanf(token,"%c",&chain)!=1) )
                  {
                     printf("Error 6: reading domain boundary file %s\n",cathfile);
                     fclose(fp);
                     exit(1);
                  }
                  ...
                  if ( !token || (sscanf(token,"%f",&res)!=1) )
                  {
                     printf("Error 7: reading domain boundary file %s\n",cathfile);
                     fclose(fp);
                     exit(1);
                  }

                  subres=token[strlen(token)-1];

                  subresnum=0.00;

                  if(subres!=' ' && subres!='1' && subres!='2' && subres!='3' && subres!='4' && subres!='5' && subres!='6' && subres!='7' && subres!='8' && subres!='9' && subres!='0')
                  {
                     subresnum=subres-64;
                     subresnum=subresnum/100;
                  }
    
                  res=res+subresnum;
        
                  domain[i].residue[j]=res;

                  token = strtok(NULL,sepstring);

                  if ( !token || (sscanf(token,"%c",&chain)!=1) )
                  {
                     printf("Error 8: reading domain boundary file %s\n",cathfile);
                     fclose(fp);
                     exit(1);
                  }
                  ...
                  if ( !token || (sscanf(token,"%f",&res)!=1) )
                  {
                     printf("Error 9: reading domain boundary file %s\n",cathfile);
                     fclose(fp);
                     exit(1);
                  }
   ...
   else if(code[5]=='0')
   {
      *domains_total=1;
      ...
      domain=(struct DOM *) calloc(*domains_total,sizeof(struct DOM));

      strncpy(domain[0].protein_id,code,5);
      domain[0].dom_number=0;
      domain[0].segments=1;
      domain[0].residue[0]=-2.5;
      domain[0].residue[1]=-2.5;
   }
...
struct HELIX* read_helices(FILE *fptr, struct DOM *domain, int *helices_total, int *domains_total, char *code)
{
   /* Variables */
   ...
   /* get the correct domain number */

   dom[0]=code[5];
   dom[1]='\0';

   for(g=0;g<*domains_total;g++)
   {
      if(domain[g].dom_number==atoi(dom))
      break;
   } 

   /* h and h+1 represents first and last residues of domain segment */
                     
   for(h=0; h<domain[g].segments*2; h+=2)
   {
      segment=FALSE;
      ...
      while(!feof(fptr))
      {
         ...
         if(line[13]!='!')
         {
            ...
            if(line[11]==code[4])             /* if chain label is right */
            {
               ...
               if((res_number==domain[g].residue[h]) || (domain[g].residue[h]==-2.5)) 
               {
                  segment=TRUE;    /* the domain segment has started */
               }   
               ...
               else if((segment==FALSE) && (previous_structure=='I' || previous_structure=='G' || previous_structure=='H'))
               {
               ...
               if(res_number==domain[g].residue[h+1])
               {
                  segment=FALSE;  /* the domain segment has ended */
               }   
            }
         }
      }
...
struct ATOM** read_atom(FILE *fp, struct HELIX *helix, int *helices_total, int *helices_atom_total, char *code)
{
   /* Variables */
   ...
   */
   ...
   while(!feof(fp))
   {
      ...
      if((line[13]!='H') && (!strncmp(line,"ATOM  ",6) || !strncmp(line,"HETATM",6)))
      {
         ...
         if((chain==code[4]) && (strcmp(resname,"HOH")))        
         {
      ...
      if(!strncmp(line,"END",3))  /* if end of pdb is reached and helices are not completed, rewind back to beginning */
      {
         rewind(fp);
         check++;
         ...
         if(check==10)
         {
            printf("\n** Error in PDB file read atom loop **\n");
            break;     /* only rewind 10 times before breaking */
         }
      }
   }
...
struct DISTANCE** residue_distance(int helix1, int helix2, struct HELIX *helix, struct HELIXPAIR **helix_pair)
{
   ...
   if(i!=j)
   {
      ...
      /* and the distance between their midpoint C-alphas is less than 25 Angstroms and they are both in the same chain */

      m=helix[i].residues_total/2;    /* middle C-alpha of helix 1 */
      n=helix[j].residues_total/2;    /* middle C-alpha of helix 2 */

      if((residue_residue[m][n].distance<=25.0) && (helix[i].chain[4]==helix[j].chain[4])) 
      {
         for(g=0; g<helix[i].residues_total; g++)
         {
            for(h=0; h<helix[j].residues_total; h++)
