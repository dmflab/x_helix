...
// ISSUE:  close contact code determines helix distance based on infinite extension
...
//         likely is the source of the helix distances being too small.
// ISSUE:  some helix distances are maybe too small (6.7A, for example). Gly/Gly packing
//         gives ~6.4A axial distance (ballpark). 'close' packing may be arising
//         due to use of average helix axis over the whole (possibly curved) helix.
...
// end of cathfile input read and process loop
    ...
    if((fp_pyaxis=fopen(pymol_axis, "a+"))==NULL)
    {
        printf("\n\n** Error appending to file '%s'!",pymol_axis);
        exit(1);
    }
    fprintf(fp_pyaxis,"set dash_gap, 0, cont*\n");
    fprintf(fp_pyaxis,"set dash_radius, 0.40\n");
    fprintf(fp_pyaxis,"set dash_round_ends, 0\n");
    fprintf(fp_pyaxis,"set dash_color, 0xffcc00, dist*\n");
    fprintf(fp_pyaxis,"hide labels, dist*\n");
    fclose(fp_pyaxis);
...
void get_bending_angle(int helix_number, struct HELIX *helix)
{
   ...
   if((fpo=fopen(output_bending, "a+"))==NULL)
   {
    ...
    {
        if((fp_pyaxis=fopen(pymol_axis, "w"))==NULL)
        {
