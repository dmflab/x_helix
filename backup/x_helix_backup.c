...
// dmf 6.27.17 
// DO!   :very basic listing output, need graphical translation of some sort
// 		possibly could use axis.txt data to do PyMol workaround like this: 
//			pseudoatom pt1, pos=[x1, y1, z1]
//			pseudoatom pt2, pos=[x2, y2, z2]
//			distance /pt1, /pt2
//			set dash_gap, 0
// 		see: https://sourceforge.net/p/pymol/mailman/message/25795427/
// DO!   : output files are not named per the files being processed (e.g. 
//         should have "1ffh_output.txt" or whatever, not just "output.txt") 
// ISSUES: turn off the debugging statements - problem from DSSP webserver line
//         length issue, but OK now
// ISSUES: hardwired location of ancillary files [need to work out a usage protocol]
// ISSUES: some helix distances are maybe too small (6A, for example)
...
// 3) change variable output_file1[25] to output_packing[28], and substitute in code throughout
// 4) change variable output_file2[20] to output_shape[24], and substitute in code throughout
// 5) change usage of obpdbfile, so that it allows 1xyz.pdb as a default alternate to pdb1xyz.ent
// 6) added global flag new_open to control contact file opens (was flagged on helix # -> bug if 
...
int main(void)     
{
   ...
   char output_helices[20]="helices.txt";
   char output_packing[25]="helix_packing_pair.txt";
   char output_shape[20]="helix_shape.txt";
   char pdb_id[5];
   char temp_pdb[5]="";
   char dsspfile[50];
   char PDBDIR[39];
   ...
   if((fpo1=fopen(output_packing, "w"))==NULL)
   {
   ...
   if((fpo2=fopen(output_shape, "w"))==NULL)
   {
...
// dmf 6.28.17 change to allow 1xyz.pdb as a default alternate to pdb1xyz.ent
         ...
         if((fpo=fopen(output_helices, "w"))==NULL)
         {
         ...
         if((fpo1=fopen(output_packing, "a+"))==NULL)
         {
            printf("\n\n** Error writing to file '%s'!",output_packing);
            exit(1);
         }
         ...
         if((fpo2=fopen(output_shape, "a+"))==NULL)
         {
            printf("\n\n** Error writing to file '%s'!",output_shape);
            exit(1);
         }
...
#ifdef DEBUG 
	printf("\n current_structure from dssp line %c", current_structure); 
	printf("\n the line %s", line); 
// doing something funny - first line is "HAIN" (which is from CHAIN), and is doubling up so that
// each line is being processed twiced - this doubles the length of the helix, so there's a crash... 
// to figure out
#endif
...
#ifdef DEBUG
        printf("\n current_structure changes to %c", current_structure);
        printf("\n previous dssp helix_no %d", i);
        printf("\n previous dssp residues_total %d", k);
#endif
...
#ifdef DEBUG
	printf("\n\t helix[i].ca_coord[k][0]: %f", helix[i].ca_coord[k][0]); 
	printf("\n\t helix[i].ca_coord[k][1]: %f", helix[i].ca_coord[k][1]); 
	printf("\n\t helix[i].ca_coord[k][2]: %f", helix[i].ca_coord[k][2]); 
	printf("\n\t k: %d", k); 
	printf("\n\t and current_residue: %f\n", current_residue); 
#endif
...
{  
   ...
   if(i==0)
   {
      if((fpo=fopen(output_axis, "w"))==NULL)
      {
   ...
   {
      if((fpo=fopen(output_axis, "a+"))==NULL)
      {
...
#ifdef DEBUG
	printf("\n helix i %d", i); 
	printf("\n residues_total %d",helix[i].residues_total); 
	printf("\n loop index j %d", j); 
	printf("\n coord dump %f", helix[i].ca_coord[j][0]);
	printf("\n coord dump %f", helix[i].ca_coord[j][1]);
	printf("\n coord dump %f", helix[i].ca_coord[j][2]);
	printf("\n coord dump %f", helix[i].ca_coord[j+1][0]);
	printf("\n coord dump %f", helix[i].ca_coord[j+1][1]);
	printf("\n coord dump %f", helix[i].ca_coord[j+1][2]);
	printf("\n coord dump %f", helix[i].ca_coord[j+2][0]);
	printf("\n coord dump %f", helix[i].ca_coord[j+2][1]);
	printf("\n coord dump %f", helix[i].ca_coord[j+2][2]);
	printf("\n coord dump %f", helix[i].ca_coord[j+3][0]);
	printf("\n coord dump %f", helix[i].ca_coord[j+3][1]);
	printf("\n coord dump %f\n", helix[i].ca_coord[j+3][2]);
#endif
...
void get_bending_angle(int helix_number, struct HELIX *helix)
{
   ...
   int i,j,k=0,l;
   FILE *fpo;
   char output_bending[20]="axis.txt";
   double angle;
   double max_angle=0;
   double pi;
   ...
   if((fpo=fopen(output_bending, "a+"))==NULL)
   {
      printf("\n\n** Error appending to file '%s'!",output_bending);
      exit(1);
   }
...
void fit(int helix_number, struct HELIX *helix)
{
   ...
   if(i==0)
   {
      if((fpo=fopen(output_geom, "w"))==NULL)
      {
   ...
   {
      if((fpo=fopen(output_geom, "a+"))==NULL)
      {
...
struct DISTANCE** atom_distance(int helix1, int helix2, struct HELIX *helix, struct ATOM **helix_atom, struct HELIXPAIR **helix_pair)
{
   ...
   int switch_end;
   FILE *fpo;
   char output_contact[20]="contact.txt";
...
// dmf 6.27.17 use a global flag
   if (new_open) 
   {
      if((fpo=fopen(output_contact, "w"))==NULL)
      {
   ...
   {
      if((fpo=fopen(output_contact, "a+"))==NULL)
      {
...
#ifdef DEBUG
  	fprintf(fpo,"\t\tTesting g %d and h %d\n", g, h); 
#endif
